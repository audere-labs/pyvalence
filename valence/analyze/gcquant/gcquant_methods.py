...
class GCMethods(object):
    ...
    def _compiled(self, lib, area):
        ...
        returndf = lib_grouped.apply(
            lambda x: self._matchiter(
                x.reset_index(),
                area_grouped.get_group(x.name).reset_index())
        )
        returndf.drop(['header=', 'pct_area', 'ref'],
                      axis=1,
                      inplace=True)
        return returndf
    ...
    #                                value_vars=cal_files)
    #     standards_melted.columns = ['library_id', 'key', 'cal_conc']
    #     return_df = pd.merge(matcheddf.reset_index(),
    #                          standards_melted,
    #                          how='left', on=['library_id', 'key'])
    #     return return_df.dropna(subset=['cal_conc'])

    def cal_curves(matched_cal_conc):
        """ take a matched calibration concentration dataframe
            (matched_cal_conc) and does a linear regression and returns
            a dataframe of the library_ids with linregress stats and the
            min/max areas which is the range for which the calibration curve
            can (should only) be used. this is only for cases where each
            molecule has a calibration curve i.e. tic and/or fid
        """
        b = (matched_cal_conc.groupby('library_id')
                             .apply(lambda a: linregress(a.area, a.cal_conc))
                             .apply(pd.Series)
                             .reset_index())
        b.columns = ['library_id', 'slope', 'intercept',
    ...
    def _stdcurves(self, compiled, standards):
        """ takes in compiled dataframe of species compiled with areas and
    ...
    def _concentrations(self, compiled, stdcurves):
        ...
        return_df.drop(['rvalue', 'pvalue', 'stderr'], axis=1, inplace=True)
